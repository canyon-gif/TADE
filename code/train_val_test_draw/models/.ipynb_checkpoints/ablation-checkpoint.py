...
class SingleModal(nn.Module):
    def __init__(self, input_dim, dim, depth, heads, attn_dropout, ff_dropout,
                 k_sum, k_prod, use_prompts=True, qk_relu=False):
        super().__init__()
        ...
        self.layers = nn.ModuleList([
            ArithmeticBlock(token_num=input_dim, heads=heads, dim=dim,
        ...
        fusion_input_dim = (k_sum + k_prod) * dim
        self.fusion_layer = nn.Linear(fusion_input_dim, dim)
    ...
    def forward(self, x, label=None):
        ...
        x = x.view(x.size(0), -1)  # (B, (k_sum + k_prod)*dim)
